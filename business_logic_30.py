...
def find_mismatches(df):
    ...
    for index, row in df.iterrows():
        try:
            ...
            calculated_selling_pax = max(safe_get_value(row, 'ordered pax/vendor mg'), safe_get_value(row, 'actual consumption'), safe_get_value(row, 'client mg/pre order'))
